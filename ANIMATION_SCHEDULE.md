...
| 场景 | 默认动画 | 切换后动画 | 备选/兜底 |
|---|---|---|---|
| IDLE | 待机呼吸休闲 | 83 个随机动画按频率触发 | 工作、余额和点击动画不进入随机池 |
| THINKING | 工作状态-思考冒泡 | 原状态动画池 | cue 播完后继续现有状态动画 |
| WORKING | 工作状态-忙碌点按 / 工作状态-清点归档 | 原状态动画池 | 由工具开始或完成 cue 区分 |
| WAITING | 工作状态-原地踱步张望 | 原状态动画池 | 等待批准时触发 |
| SUCCESS | 工作状态-雀跃庆祝 | 待机呼吸休闲 | 专属片段播放一次 |
| ERROR | 工作状态-垂头叹气冒汗 | 待机呼吸休闲 | 专属片段播放一次 |
| DISCONNECTED | 用户选择的退出动画路径 |  | 由离别协调器独占 |

## 4. 动画清单与触发场景

动画总数：106；唯一动画：106。分类为：待机 1、转向 1、移动 3、点击 5、拖拽 1、随机 83、工作 6、余额 6。
...
- IDLE 随机池只包含 83 个普通随机动画；6 个工作动画由 cue 触发，6 个余额动画只允许右键手选，5 个点击动画只响应点击。
