...
- 桌面置顶透明窗口，播放 106 个 640×360 透明 WebM 动画（待机 / 转向 / 移动 / 点击回应 / 拖拽 / 随机 / 工作状态 / 余额）。
...
- Codex 工作状态通过固定 `animationCue` 映射到 6 个专属动画；余额动画仅供手选，不自动推测。
- Windows 支持角色区域手套抓取光标；播放器按需创建，包含预测预热、8 MiB 首帧 LRU、单线程常驻 FFmpeg、边界回收和高刷物理计时。
