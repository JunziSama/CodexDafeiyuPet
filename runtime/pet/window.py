...
from PySide6.QtGui import QActionGroup, QBitmap, QColor, QImage, QPainter, QPixmap
...
def wander_target_y(start_y: float, top: float, bottom: float, height: float,
    ...
    return movement_mod.wander_target_y(start_y, top, bottom, height, margin, rnd=rnd)
...
class PetWindow(QWidget):
    ...
    animation_finished = Signal(str)  # 外部状态调度器可在完整片段结束后接管下一段
    animation_started = Signal(str, str)  # 动画名、来源；让外部调度器跟随窗口真实状态
    visibility_requested = Signal(bool)  # 增强控制器持久化手动显示/隐藏
    bubble_dismissed = Signal()  # 气泡实际消失边界，供同步退场使用
    ...
    def __init__(self, lib: MovieLibrary, config: Config) -> None:
        ...
        self.horizontal_auto_move_only: bool = bool(config.get('horizontal_auto_move_only', False))
        self.window_snap_enabled: bool = bool(config.get('window_snap_enabled', False))
        self.window_snap_distance: int = int(config.get('window_snap_distance', 20))
        ...
        self.idle_animation_frequency = str(config.get('idle_animation_frequency', 'balanced'))
        self.movement_frequency = str(config.get('movement_frequency', 'balanced'))
        self.exit_animation_mode = str(config.get('exit_animation_mode', 'farewell_only'))
        self._next_idle_action_at = 0.0
        self._reset_idle_action_deadline()
        # Legacy settings code may still call set_animation_gap(); the timer is
        # retained as a harmless compatibility shim while v4 uses named levels.
        self.animation_gap_seconds: float = 0.0
        ...
        self._speech_bubble.dismissed.connect(self.bubble_dismissed.emit)
        ...
        self.no_move: bool = self.movement_frequency == 'off'
        self.movie = None
        ...
        self._interaction_locked = False         # Codex 告别期间禁止启动新的交互动画
    ...
    def _reset_idle_action_deadline(self, now: float | None = None) -> None:
        now = time.monotonic() if now is None else float(now)
        interval = IDLE_ANIMATION_INTERVALS.get(self.idle_animation_frequency, 45.0)
        self._next_idle_action_at = float('inf') if interval is None else now + float(interval)

    def idle_action_due(self, now: float | None = None) -> bool:
        now = time.monotonic() if now is None else float(now)
        interval = IDLE_ANIMATION_INTERVALS.get(self.idle_animation_frequency, 45.0)
        return interval is not None and (float(interval) <= 0 or now >= self._next_idle_action_at)

    def mark_idle_action(self, now: float | None = None) -> None:
        self._reset_idle_action_deadline(now)

    def movement_probability(self) -> float:
        return float(MOVEMENT_PROBABILITIES.get(self.movement_frequency, 0.3))

    def set_idle_animation_frequency(self, value: str) -> None:
        value = str(value)
        self.idle_animation_frequency = value if value in IDLE_ANIMATION_FREQUENCIES else 'balanced'
        self.cfg.set('idle_animation_frequency', self.idle_animation_frequency)
        self.cfg.save()
        self._reset_idle_action_deadline()

    def set_movement_frequency(self, value: str) -> None:
        value = str(value)
        self.movement_frequency = value if value in MOVEMENT_FREQUENCIES else 'balanced'
        self.no_move = self.movement_frequency == 'off'
        self.cfg.set('movement_frequency', self.movement_frequency)
        self.cfg.save()
        if self.no_move and self._move_plan is not None:
            self._cancel_move()
            if self.idles:
                self._switch(self._pick(self.idles), origin='idle')  # 打断进行中的移动

    def set_exit_animation_mode(self, value: str) -> None:
        value = str(value)
        self.exit_animation_mode = value if value in EXIT_ANIMATION_MODES else 'farewell_only'
        self.cfg.set('exit_animation_mode', self.exit_animation_mode)
        self.cfg.save()

    def set_no_move(self, on: bool) -> None:
        """Compatibility setter for legacy callers."""
        self.set_movement_frequency('off' if on else 'balanced')

    def set_horizontal_auto_move_only(self, on: bool) -> None:
        """Restrict autonomous and menu-triggered walking to the current y."""
        self.horizontal_auto_move_only = bool(on)
        self.cfg.set('horizontal_auto_move_only', self.horizontal_auto_move_only)
        self.cfg.save()
        if self.horizontal_auto_move_only and self._move_plan is not None:
            # Re-plan from the current position so enabling the option takes
            # effect immediately without constraining user drag/throw physics.
            move_name = self.anim if self.anim in self.moves else None
            self._cancel_move()
            if move_name is not None:
                self._try_move(move_name)

    def set_window_snap_enabled(self, on: bool) -> None:
        self.window_snap_enabled = bool(on)
        self.cfg.set('window_snap_enabled', self.window_snap_enabled)
        self.cfg.save()

    @staticmethod
    def _rect_from_qrect(rect: QRect) -> snapping_mod.Rect:
        return snapping_mod.Rect(
            float(rect.x()), float(rect.y()),
            float(rect.x() + rect.width()), float(rect.y() + rect.height()),
        )

    def _ordinary_window_rects(self) -> list[snapping_mod.Rect]:
        if sys.platform != 'win32':
            return []
        spaces = snapping_mod.screen_spaces(QApplication.screens())
        try:
            own_hwnd = int(self.winId())
        except Exception:
            own_hwnd = 0
        return snapping_mod.enumerate_ordinary_windows(
            spaces, own_pid=os.getpid(), excluded_hwnds=([own_hwnd] if own_hwnd else [])
        )

    def _apply_window_snap(self) -> bool:
        """Snap visible character bounds after a completed positioning action."""
        if (not self.window_snap_enabled or self._dragging
                or self._physics_mode in {'drag', 'throw'}):
            return False
        screen = self._screen_available()
        if screen is None:
            return False
        screen_rect = self._rect_from_qrect(screen.availableGeometry())
        visible = self._rect_from_qrect(self.visible_content_rect())
        dx, dy = snapping_mod.snap_offset(
            visible, screen_rect, self._ordinary_window_rects(), self.window_snap_distance
        )
        snapped = visible.translated(dx, dy)
        clamp_x, clamp_y = snapping_mod.clamp_offset(snapped, screen_rect)
        dx += clamp_x
        dy += clamp_y
    ...
    # ================================================================ 播放
    def _switch(self, name: str, origin: str = 'idle') -> None:
    ...
    def hold_finished_animation(self) -> None:
        """停在当前末帧，不让窗口自身的兼容随机链继续切换。"""
        self._ended_fired = False

    def interrupt_current_animation(self) -> None:
        """Stop playback immediately at a synchronized shutdown boundary."""
        self._cancel_move()
        if self.movie is not None:
            self.movie.stop()

    def _play_menu_animation(self, name: str) -> None:
        if not self._interaction_locked:
            self._switch(name, origin='menu')
    ...
    def _on_anim_ended(self, name: str) -> None:
        finished_movie = self.movie
        self.animation_finished.emit(name)
        # 外部调度器若已调用 _switch()，即使选中了同名动画也会重置
        # _ended_fired；此时不再执行本窗口的随机链，避免连续切换两次。
        if self.movie is not finished_movie or not self._ended_fired:
            return
        ...
        if name == self.drag or name in self.clicks:
            ...
            if self.idles:
                self._switch(self._pick(self.idles), origin='idle')
            return
        self._pick_next()
    ...
    def _play_animation_gap_step(self) -> None:
        ...
        if pool:
            self._switch(self._pick(pool, exclude=self.anim), origin='idle')

    def _on_animation_gap_timeout(self) -> None:
        self._animation_gap_active = False

    def _pick_next(self) -> None:
        """v4 idle scheduler: movement and autonomous actions are independent."""
        now = time.monotonic()
        probability = self.movement_probability()
        if probability > 0 and random.random() < probability and self._try_move_with_turnaround():
            return
        if self.idle_action_due(now):
            pool = self.turns + self.acts
            if pool:
                self.mark_idle_action(now)
                self._switch(self._pick(pool, exclude=self.anim), origin='idle')
                return
        pool = self.idles or self.turns or self.acts
        if pool:
            self._switch(self._pick(pool, exclude=None), origin='idle')
    ...
    # ================================================================ 移动
    def _try_move(self, name: str | None = None, origin: str = 'movement') -> bool:
        ...
        start_y = self.y()
        target_y = movement_mod.target_y(
            self.horizontal_auto_move_only, start_y,
            float(avail.top()), float(avail.bottom()),
            float(self._h), float(catalog.MOVE_MARGIN), rnd=random)
        move_name = name or self._pick(self.moves)
        duration = self.lib.duration(move_name)
        self._switch(move_name, origin=origin)
        self._move_plan = {
            'start_x': self.x(),
    ...
    def _try_move_with_turnaround(self, name: str | None = None, origin: str = 'movement') -> bool:
        """Walk in the current direction, reversing once when blocked by an edge."""
        if self._try_move(name, origin=origin):
            return True
        original = self.facing
        self.facing = 'right' if original == 'left' else 'left'
        if self._try_move(name, origin=origin):
            return True
        self.facing = original
        return False
    ...
    def _trigger_move(self, name: str) -> None:
        ...
        self._cancel_move()
        self._cancel_animation_gap()
        if not self._try_move(name, origin='menu'):
            self._switch(name, origin='menu')  # 贴边放不下：原地播放走路姿态，不位移
    ...
    def mouseMoveEvent(self, event) -> None:  # noqa: N802
        ...
        if not self._dragging:
            ...
            self._dragging = True
            if self.drag:
                self._switch(self.drag, origin='drag')  # 进入拖拽：播放悬空反馈动画
    ...
    def mouseReleaseEvent(self, event) -> None:  # noqa: N802
        ...
        if was_dragging:
            ...
            if self.drag_physics:
                ...
                snap_after_release = True
            if self.idles:
                self._switch(self._pick(self.idles), origin='idle')  # 回待机缓冲
            if sys.platform == 'win32':
        ...
        if was_dragging and snap_after_release:
            self._apply_window_snap()
            self._save_position()
        event.accept()
    ...
    def _on_click(self) -> None:
        ...
        if self._just_dragged or self._interaction_locked:
            return
        if not self.clicks:
            return
        ...
        self._switch(self._pick(self.clicks), origin='click')
        self._start_squash()
        if sys.platform == 'win32':
    ...
    def contextMenuEvent(self, event) -> None:  # noqa: N802
        ...
        if self.idles:
            ...
            for n in self.idles:
                m_idle.addAction(n, lambda n=n: self._play_menu_animation(n))
        if self.turns:
            m_turn = m_anim.addMenu('转向')
            for n in self.turns:
                m_turn.addAction(n, lambda n=n: self._play_menu_animation(n))
        ...
        animation_frequency_menu = menu.addMenu('动画频率')
        animation_frequency_group = QActionGroup(animation_frequency_menu)
        animation_frequency_group.setExclusive(True)
        for key, label in (
            ('continuous', '一直播放'),
            ('frequent', '频繁（约15秒）'),
            ('balanced', '适中（约45秒）'),
            ...
            action = animation_frequency_menu.addAction(label)
            action.setCheckable(True)
            action.setChecked(self.idle_animation_frequency == key)
            action.triggered.connect(lambda checked=False, key=key: self.set_idle_animation_frequency(key))
            animation_frequency_group.addAction(action)

        movement_frequency_menu = menu.addMenu('移动频率')
        movement_frequency_group = QActionGroup(movement_frequency_menu)
        movement_frequency_group.setExclusive(True)
        for key, label in (
            ('continuous', '一直移动'),
            ('frequent', '频繁（60%）'),
            ('balanced', '适中（30%）'),
            ...
            action = movement_frequency_menu.addAction(label)
            action.setCheckable(True)
            action.setChecked(self.movement_frequency == key)
            action.triggered.connect(lambda checked=False, key=key: self.set_movement_frequency(key))
            movement_frequency_group.addAction(action)
        movement_frequency_menu.addSeparator()
        horizontal_move_act = movement_frequency_menu.addAction('仅水平移动')
        horizontal_move_act.setCheckable(True)
        horizontal_move_act.setChecked(self.horizontal_auto_move_only)
        horizontal_move_act.setEnabled(self.movement_frequency != 'off')
        horizontal_move_act.toggled.connect(self.set_horizontal_auto_move_only)

        exit_mode_menu = menu.addMenu('Codex 退出方式')
        exit_mode_group = QActionGroup(exit_mode_menu)
        exit_mode_group.setExclusive(True)
        for key, label in (
            ('current_then_farewell', '当前动画 + 告别动画'),
            ('farewell_only', '仅告别动画'),
            ('current_only', '仅当前动画'),
            ('bubble_sync', '随结束气泡消失'),
        ):
            action = exit_mode_menu.addAction(label)
            action.setCheckable(True)
            action.setChecked(self.exit_animation_mode == key)
            action.triggered.connect(lambda checked=False, key=key: self.set_exit_animation_mode(key))
            exit_mode_group.addAction(action)
        ...
        window_snap_act = menu.addAction('窗口吸附')
        window_snap_act.setCheckable(True)
        window_snap_act.setChecked(self.window_snap_enabled)
        window_snap_act.toggled.connect(self.set_window_snap_enabled)
        ...
        menu.addAction('隐藏桌宠', lambda: self.visibility_requested.emit(False))
        ...
        # EAC integration intentionally removes the standalone update/download
        # and launcher actions; the host plugin owns lifecycle and status.
        menu.addSeparator()
        menu.addAction('退出', self._request_quit)
        menu.ensurePolished()
        hint = menu.sizeHint()
        pet_rect = self.visible_content_rect()
        screen = QApplication.screenAt(event.globalPos()) or self.screen()
        available = screen.availableGeometry()
        x, y = menu_position_mod.above_pet(
            (pet_rect.x(), pet_rect.y(), pet_rect.width(), pet_rect.height()),
    ...
    def show_bubble(self, text: str, duration_ms: int = 3200, state: str = "IDLE") -> None:
        """向桌宠头顶冒泡提示（app 层反馈用，非侵入）。"""
        self._speech_bubble.show_text(text, self.visible_content_rect(), duration_ms, state=state)

    def refresh_pet_settings(self) -> None:
        self.idle_animation_frequency = str(self.cfg.get('idle_animation_frequency', 'balanced'))
        self.movement_frequency = str(self.cfg.get('movement_frequency', 'balanced'))
        self.exit_animation_mode = str(self.cfg.get('exit_animation_mode', 'farewell_only'))
        self.no_move = self.movement_frequency == 'off'
        self._reset_idle_action_deadline()
        self._cancel_animation_gap()
    ...
    def set_animation_gap(self, seconds: float) -> None:
        self.set_idle_animation_frequency(migrate_animation_gap(seconds))
